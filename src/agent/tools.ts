...
export const toolDefinitions: ToolDefinition[] = [
  defineTool(
    "proof_pattern_engine",
    "Plan and record formula-level proof transformations before doing explicit computation. Use this when the next step is to choose, normalize, apply, compile, or audit a proof move rather than simplify a chosen expression. When the problem already supplies a proposed transformation, prefer compile to create a traceable ledger of the move, bindings, and missing side conditions instead of waiting for a registered pattern to match. Use register only when the user explicitly asks to add a reusable transform to the engine, not during ordinary problem solving. It returns transformation candidates, side-condition status, missing assumptions, and proof-state traces. It is not a whole-paper verifier and does not replace explicit Wolfram checks of the resulting formulas.",
    {
